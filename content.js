class GoogleMeetEmojiPicker {
  ...
  createEmojiPicker() {
    ...
    // Common emoji categories - expanded list
    ...
    // Create category tabs (horizontal)
    ...
    // Create tabs and set up category switching
    Object.keys(emojiCategories).forEach(category => {
      const tab = document.createElement('button');
      tab.textContent = this.getCategoryEmoji(category);
      tab.title = category;
      tab.style.cssText = `
        background: none;
        border: none;
      ...
      tab.addEventListener('click', (e) => {
        e.stopPropagation();
        this.showCategory(emojiGrid, emojiCategories[category]);
    ...
    this.showCategory(emojiGrid, emojiCategories['Smileys & People']);
    if (tabContainer.firstChild) {
      tabContainer.firstChild.style.borderBottomColor = '#1a73e8';
    }

    // Assemble the picker
    emojiGridContainer.appendChild(emojiGrid);
    pickerContent.appendChild(tabContainer);
